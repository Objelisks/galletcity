...
initialize(map, W, H);
initialize(meta, W, H, () => ({height:0, clip:1}));
...
let set = (x, y, w, h, tile_props, target=map) => {
  for(let i=x; i<x+w; i++) {
    for(let j=y; j<y+h; j++) {
      Object.keys(tile_props)
        .forEach(prop => target[i][j][prop] = tile_props[prop]);
}}};
let toggles = {
  draw: true,
  road: false,
...
let modes = {
  draw: (x, y, value, target) => (target!=map || !!meta[x][y].clip) ? set(x, y, 1, 1, {s:value.s}, target) : null,
  road: (x, y, value, target) => target==map ? set(x, y, 1, 1, {road:true}, meta) : null,
  ...
  clip: (x, y, value, target) => target==map ? set(x, y, 1, 1, {clip:value}, meta) : null,
};
let toggle = (what) => {
...
function alchemy(x, y, val, target=map) {
  if(!val.s) return val;
...
function drawtarget(pattern, x, y) {
  pattern.forEach((row, i) =>
    row.forEach((val, j) =>
      val.s !== 0 && drawrect(x+i, y+j, 1, 1, val)));
}

function matchat(x, y, pattern, target=map) {
  return pattern.every((row, j) =>
    row.every((val, i) =>
      safe(x+i, y+j, target) && (target[x+i][y+j].s == val || val == -1)
    ));
}

function replaceinshape(x, y, pattern, replace, val, target=map) {
  pattern.forEach((row, j) => {
    row.forEach((col, i) => {
      if(safe(x+i, y+j, target) && !empty(x+i, y+j, target) && col == replace) {
        drawrect(x+i, y+j, 1, 1, val, target);
}})})}

function regex(pattern, replace, val, target=map) {
  for(let y=0; y<H; y++) {
    for(let x=0; x<W; x++) {
      if(matchat(x, y, pattern, target)) {
        let matchval = val instanceof Function ? val(x, y) : val;
        replaceinshape(x, y, pattern, replace, matchval, target);
}}}}
...
function drawrail() {
  ...
  drawrect(0, y, W, 1, (i,j) => meta[i][j].road ? tile(127) : tile(126)); // top rail
  drawrect(0, y+1, W, 1, (i,j) => meta[i][j].road ? tile(135) : tile(134)); // bot rail
...
function building1(x, y, w, h) {
  ...
  drawrect(x+1, b-(tall-2), w-1, tall-2, tile(153)); // windows
...
function building2(x, y, w, h) {
  ...
  let lightgray = [17, 21, 24, 25, 33, 32, 32, 32, 32];
  let lightgrey_wall = [64, 65, 76];
  let roofpalettes = [
    [lightgray, lightgrey_wall, lightgrey_wall, lightgray], // light grey
...
function scaffold(x, y, w, tall) {
  drawrect(x, y-tall, w, 1, tile(104));
  drawrect(x, y-tall+1, w, tall-1, tile(112));
}

function construction(x, y, w, h) {
  let depth = rand(3)+2;
  let floor = [];
  initialize(floor, w, h+20);
  drawrect(1, 10, w-1, h, tile(139000), floor);
  drawrect(0, 10, 1, h, tile(108), floor);
  
  regex([[139000]], 139000, () => (i,j) => Math.random() > 0.9 ? () => {
    drawrect(i,j+1,1,1,tile(116), floor);
    drawrect(i+1,j+1,1,1,tile(108), floor);
    drawrect(i+1,j,1,1,tile(109), floor);
    return tile(58);
  }: floor[i][j], floor); // pillars
  regex([[58]], 58, (height=rand(6)+1) => (i,j) => { 
    drawrect(i,j-height,1,height,tile(50000),floor);
    drawrect(i,j-height-1,1,1,tile(6),floor);
    return tile(58);
  }, floor); // pillars
  regex([[139000],[139000]], 139000, (pi,pj) => Math.random() > 0.9 ? (i,j) => j==pj ? tile(122) : tile(138) : (i,j) => floor[i][j], floor); // logs
  regex([[139000],[139000]], 139000, (pi,pj) => Math.random() > 0.9 ? (i,j) => j==pj ? tile(121) : tile(137) : (i,j) => floor[i][j], floor); // pipes
  regex([[137],[139000]], 139000, () => Math.random() > 0.5 ? (i,j) => { drawrect(i,j-1,1,1,tile(129),floor); return tile(137)} : tile(139000), floor); // extend logs
  regex([[138],[139000]], 139000, () => Math.random() > 0.5 ? (i,j) => { drawrect(i,j-1,1,1,tile(130),floor); return tile(138)} : tile(139000), floor); // extend pipes
  regex([[139000]], 139000, () => (i,j) => Math.random() > 0.4 ? rtile([123, 131, 132, 124, 140]) : floor[i][j], floor); // dirty dirt
  
  toggle({draw:false, clip:true});
  drawrect(0, 0, W, H, 0);
  drawrect(x, y+h-20, w, 20, 1);
  toggle({draw:true, clip:false});
  
  scaffold(x, y+depth, w, depth);
  drawrect(x, y+1, 1, 1, tile(120));
  drawrect(x, y+2, 1, Math.max(0, depth-2), tile(128));
  drawtarget(floor, x, y+depth-10);
  
  //todo: draw heights, and shade floor
...
function drawbuilding(x, y, w, h, large=false) {
  ...
  let building = pick(large ? largebuildings : buildings)[0];
  building(x, y, w, h);
  return building;
}
...
function trytoplace(x, y, large) {
 ...
 while((empty(x+bw, y) || empty(x, y+bh)) && bw < rand(10)+3 && bh < rand(10)+3) {
  ...
  if(free(x, y, bw, bh)) {
    let sidealley = false, fillright = false, filldown = false;
    ...
    let outbuild = drawbuilding(x, y, bw+(fillright?fillright:0)-(sidealley?1:0), bh+(filldown?filldown:0), large);
    if(sidealley && outbuild != park) {
      drawalley(x+bw-1, y, 1, bh+(filldown?filldown:0));
    }
  } else {
    return false;
  }
...
function placebuilding(x, y) {
  let large = Math.random() > 0.7;
  if(!trytoplace(x, y, large)) {
    trytoplace(x, y, false);
  }
}
...
function decorate_roads() {
  ...
  regex([[10],[1],[1],[1],[-1],[1],[1],[1],[1],], -1, tile(16)); // diamond
  regex([[1],[32]], 1, tile(10)); // crosswalk
  regex([[15, 1], [-1, 10]], 1, tile(8)); // stop bar
  regex([[1],[1],[1],[1],[-1],[1],[1],[1],[10]], -1, tile(16)); // diamond
...
function postshade_decorate() {
  ...
  regex([[139]], 139, () => (i,j) => Math.random() > 0.6 ? rtile([123, 131, 132, 124, 140]) : map[i][j]); // dirty dirt
  ...
  regex([[1000]], 1000, tile(1)); // hack to prevent replacement until end
  regex([[2000]], 2000, tile(2));
  regex([[5000]], 5000, tile(5));
  regex([[32000]], 32000, tile(32));
  regex([[50000]], 50000, tile(50));
